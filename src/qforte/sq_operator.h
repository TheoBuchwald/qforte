...
class SQOperator {
     ...
     *
     * All storage, printing, and input of summands in the combination takes tuples of the following
     * form: (1) coefficient (2) vector of orbital-indices of creation operators (3) vector of
     * orbital-indices of annihilation operators, All orbital indices start at zero. Index vectors
     * are lexicographic, i.e., std::tuple<1, {p, q}, {s, r}> means 1 * p^ q^ s r.
     */
  public:
    ...
    void add_term(std::complex<double> coeff, const std::vector<size_t>& cre_ops,
    ...
    /// descending.
    void canonical_order_single_term(
  ...
  private:
    ...
    void jw_helper(QubitOperator& holder, const std::vector<size_t>& operators, bool creator,
